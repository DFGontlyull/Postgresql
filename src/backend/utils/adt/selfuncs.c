...
static double eqjoinsel_inner(Oid
operator,
        VariableStatData
*vardata1,
VariableStatData *vardata2
);
static double eqjoinsel_semi(Oid
operator,
        VariableStatData
*vardata1,
VariableStatData *vardata2,
...
eqsel_internal(PG_FUNCTION_ARGS, bool negate) {
    ...
    if (IsA(other, Const))
        ...
        selec = var_eq_non_const(&vardata, operator,
    other,
            varonleft, negate);
 ...
 */
static double
        var_eq_const(VariableStatData * vardata, Oid
operator,
        Datum
constval,
bool constisnull,
...
/*
		 ...
		 */
if (
get_attstatsslot(&sslot, vardata
->statsTuple,
 ...
 */
static double
        var_eq_non_const(VariableStatData * vardata, Oid
operator,
        Node
*other,
bool varonleft,
...
HeapTupleIsValid(vardata
->statsTuple) &&
statistic_proc_security_check(vardata, opproc
->fn_oid) &&
get_attstatsslot(&sslot, vardata
->statsTuple,
STATISTIC_KIND_MCV, InvalidOid,
...
scalarltsel(PG_FUNCTION_ARGS) {
    ...
    selec = scalarineqsel(root, operator,
    isgt, &vardata, constval, consttype);
...
scalararraysel(PlannerInfo *root,
    ...
    if (rightop && IsA(rightop, Const)) {
        Datum
        arraydatum = ((Const *) rightop)->constvalue;
...
estimate_array_length(Node * arrayexpr) {
    ...
    if (arrayexpr && IsA(arrayexpr, Const)) {
        Datum
        arraydatum = ((Const *) arrayexpr)->constvalue;
...
continue;
if (
DatumGetBool(FunctionCall2Coll(&eqproc,
                               DEFAULT_COLLATION_OID,
                               sslot1.values[i],
                               sslot2.values[j])
))
...
totalsel1 +=
unmatchfreq1 *otherfreq2
/ (nd2 - sslot2.nvalues);
if (nd2 > nmatches)
totalsel1 +=
otherfreq1 *(otherfreq2
+ unmatchfreq2) /
(nd2 - nmatches);
...
totalsel2 +=
unmatchfreq2 *otherfreq1
/ (nd1 - sslot1.nvalues);
if (nd1 > nmatches)
totalsel2 +=
otherfreq2 *(otherfreq1
+ unmatchfreq1) /
(nd1 - nmatches);
 ...
 */
static double
        convert_numeric_to_scalar(Datum
value,
Oid typid,
bool *failure
)
...
if (!
lc_collate_is_c(DEFAULT_COLLATION_OID)
)
{
char *xfrmstr;
size_t xfrmlen;
 ...
 */
static double
        convert_timevalue_to_scalar(Datum
value,
Oid typid,
bool *failure
)
...
case ABSTIMEOID:
return
DatumGetTimestamp(DirectFunctionCall1(abstime_timestamp,
                                      value)
);
case DATEOID:
return
date2timestamp_no_overflow(DatumGetDateADT(value)
);
case INTERVALOID:
{
...
interval->
month *((DAYS_PER_YEAR
/ (double) MONTHS_PER_YEAR) * USECS_PER_DAY);
}
case RELTIMEOID:
return (
DatumGetRelativeTime(value)
* 1000000.0);
case TINTERVALOID:
{
 ...
 */
bool
        statistic_proc_security_check(VariableStatData * vardata, Oid
func_oid)
{
if (vardata->acl_ok)
return true;
...
string_to_const(const char *str, Oid datatype) {
    Datum
    conval = string_to_datum(str, datatype);
    Oid
    collation;
    int constlen;
...
deconstruct_indexquals(IndexPath *path) {
    ...
    forboth(lcc, path->indexquals, lci, path->indexqualcols)
    {
        ...
        if (IsA(clause, OpExpr)) {
            ...
            if (match_index_to_operand((Node * )
                linitial(rc->largs),
                        indexcol, index))
            {
                qinfo->varonleft = true;
                qinfo->other_operand = (Node * )
                rc->rargs;
            }
            else
            {
                Assert(match_index_to_operand((Node * )
                linitial(rc->rargs),
                        indexcol, index));
                qinfo->varonleft = false;
                qinfo->other_operand = (Node * )
