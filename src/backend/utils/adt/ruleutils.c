 ...
 */
static void
get_special_variable(Node *node, deparse_context *context, void *
 ...
 */
static void
resolve_special_varno(Node *node, deparse_context *context, void *
...
if (!tle)
elog(ERROR,
"bogus varattno for OUTER_VAR var: %d", var->varattno);

push_child_plan(dpns, dpns
->outer_planstate, &save_dpns);
resolve_special_varno((Node
*) tle->expr, context,
private, callback);
pop_child_plan(dpns, &save_dpns
);
...
if (!tle)
elog(ERROR,
"bogus varattno for INNER_VAR var: %d", var->varattno);

push_child_plan(dpns, dpns
->inner_planstate, &save_dpns);
resolve_special_varno((Node
*) tle->expr, context,
private, callback);
pop_child_plan(dpns, &save_dpns
...
if (!tle)
elog(ERROR,
"bogus varattno for INDEX_VAR var: %d", var->varattno);
...
varno<1 || var->varno> list_length(dpns->rtable)
)
elog(ERROR,
"bogus varno: %d", var->varno);
 ...
 */
static const char *
        get_name_for_var_field(Var * var, int
fieldno,
int levelsup, deparse_context
*context)
{
RangeTblEntry *rte;
AttrNumber attnum;
...
if (fieldno > 0 && fieldno <=
list_length(r
->colnames))
return
strVal(list_nth(r->colnames, fieldno - 1)
);
}
...
result = get_name_for_var_field((Var * )
expr, fieldno,
0, context);
...
*) var, NULL, &tupleDesc) != TYPEFUNC_COMPOSITE)
tupleDesc = lookup_rowtype_tupdesc_copy(exprType((Node * )
var),
exprTypmod((Node
*) var));
Assert(tupleDesc);
...
return
get_rte_attribute_name(rte, fieldno
);
}
...
/*
	 ...
	 */
expr = (Node * )
var;        /* default if we can't drill down */
...
set_deparse_for_query(&mydpns, rte
->subquery,
context->namespaces);

context->
namespaces = lcons(&mydpns,
                   context->namespaces);
...
new_nslist = list_copy_tail(context->namespaces,
                            ctelevelsup);
context->
namespaces = lcons(&mydpns, new_nslist);
...
/*
	 ...
	 */
if (
get_expr_result_type(expr, NULL, &tupleDesc
) != TYPEFUNC_COMPOSITE)
 ...
 */
static bool
        isSimpleNode(Node * node, Node * parentNode, int
prettyFlags)
{
if (!node)
return false;
...
return
isSimpleNode((Node
*) ((CoerceViaIO *) node)->arg,
node, prettyFlags);
case T_ArrayCoerceExpr:
return
isSimpleNode((Node
*) ((ArrayCoerceExpr *) node)->arg,
node, prettyFlags);
case T_ConvertRowtypeExpr:
return
isSimpleNode((Node
*) ((ConvertRowtypeExpr *) node)->arg,
node, prettyFlags);
...
op = get_simple_binary_op_name((OpExpr * )
node);
if (!op)
return false;
...
parentOp = get_simple_binary_op_name((OpExpr * )
parentNode);
if (!parentOp)
return false;
...
if (
is_hipriop &&is_lopriparent
)
return true;    /* op binds tighter than parent */

if (
is_lopriop &&is_hipriparent
)
return false;
 ...
 */
static void
        appendContextKeyword(deparse_context * context,
const char *str,
int indentBefore,
int indentAfter,
int indentPlus
...
(void)
get_variable((Var
*) node, 0, false, context);
break;

case T_Const:
get_const_expr((Const
*) node, context, 0);
break;
...
case T_WindowFunc:
get_windowfunc_expr((WindowFunc
*) node, context);
break;
...
'(');
get_rule_expr((Node
*) aref->refexpr, context, showimplicit);
if (need_parens)
appendStringInfoChar(buf,
')');
...
case T_FuncExpr:
get_func_expr((FuncExpr
*) node, context, showimplicit);
break;
...
->name));
get_rule_expr((Node
*) na->arg, context, showimplicit);
}
break;

case T_OpExpr:
get_oper_expr((OpExpr
*) node, context);
break;
...
appendStringInfoChar(buf,
'(');
get_rule_expr_paren(arg1, context,
true, node);
appendStringInfoString(buf,
" IS DISTINCT FROM ");
get_rule_expr_paren(arg2, context,
true, node);
if (!PRETTY_PAREN(context))
appendStringInfoChar(buf,
')');
...
appendStringInfo(buf,
"hashed %s", splan->plan_name);
else
appendStringInfoString(buf, splan
->plan_name);
if (
lnext(lc)
)
...
case T_FieldSelect:
{
FieldSelect *fselect = (FieldSelect * )
node;
Node *arg = (Node * )
fselect->
arg;
...
/*
				 ...
				 */
fieldname = get_name_for_var_field((Var * )
arg, fno,
0, context);
appendStringInfo(buf,
".%s",
...
"ROW(");
get_rule_expr((Node
*) fstore->newvals, context, showimplicit);
if (need_parens)
appendStringInfoChar(buf,
')');
}
...
appendStringInfo(buf,
" COLLATE %s",
generate_collation_name(collate
->collOid));
if (!PRETTY_PAREN(context))
appendStringInfoChar(buf,
')');
...
' ');
get_rule_expr((Node
*) caseexpr->arg, context, true);
}
foreach(temp, caseexpr
->args)
{
...
"WHEN ",
0, 0, 0);
get_rule_expr(w, context,
false);
appendStringInfoString(buf,
" THEN ");
get_rule_expr((Node
*) when->result, context, true);
}
if (!PRETTY_INDENT(context))
appendStringInfoChar(buf,
' ');
...
0, 0, 0);
get_rule_expr((Node
*) caseexpr->defresult, context, true);
if (!PRETTY_INDENT(context))
appendStringInfoChar(buf,
' ');
appendContextKeyword(context,
...
"ARRAY[");
get_rule_expr((Node
*) arrayexpr->elements, context, true);
appendStringInfoChar(buf,
']');
...
appendStringInfo(buf,
"::%s",
format_type_with_typemod(arrayexpr
->array_typeid, -1));
}
break;
...
Assert(list_length(rowexpr->args)
<= tupdesc->natts);
}
...
get_rule_expr_toplevel(e, context,
true);
sep = ", ";
}
i++;
...
appendStringInfo(buf,
"::%s",
format_type_with_typemod(rowexpr
->row_typeid, -1));
}
break;
...
"COALESCE(");
get_rule_expr((Node
*) coalesceexpr->args, context, true);
appendStringInfoChar(buf,
')');
}
break;
...
forboth(arg, xexpr
->named_args, narg, xexpr->arg_names)
{
Node *e = (Node * )
lfirst(arg);
...
get_rule_expr((Node
*) xexpr->args, context, true);
break;
case IS_XMLPARSE:
Assert(list_length(xexpr->args)
== 2);
...
appendStringInfo(buf,
" AS %s",
format_type_with_typemod(xexpr
->type,
xexpr->typmod));
...
context->
varprefix = save_varprefix;

if (iexpr->infercollid)
appendStringInfo(buf,
" COLLATE %s",
generate_collation_name(iexpr
->infercollid));
...
get_range_partbound_string(spec
->lowerdatums),
get_range_partbound_string(spec
->upperdatums));
break;
...
case T_TableFunc:
get_tablefunc((TableFunc
*) node, context, showimplicit);
break;

default:
elog(ERROR,
"unrecognized node type: %d", (int)
nodeTag(node)
);
break;
}
            ...
            case 'l':
                appendStringInfo(buf, "%s ",
                                 generate_operator_name(opno,
                                                        InvalidOid,
                                                        exprType(arg)));
                get_rule_expr_paren(arg, context, true, (Node * )
                expr);
                break;
            case 'r':
                get_rule_expr_paren(arg, context, true, (Node * )
...
if (
IsA(arg, NamedArgExpr
))
argnames = lappend(argnames, ((NamedArgExpr * )
arg)->name);
argtypes[nargs] =
exprType(arg);
nargs++;
...
get_agg_expr(Aggref * aggref, deparse_context * context,
    ...
    if (DO_AGGSPLIT_COMBINE(aggref->aggsplit)) {
        ...
        resolve_special_varno((Node * )
        tle->expr, context, original_aggref,
    ...
    if (AGGKIND_IS_ORDERED_SET(aggref->aggkind)) {
        ...
        get_rule_expr((Node * )
        aggref->aggdirectargs, context, true);
    ...
    if (aggref->aggfilter != NULL) {
        ...
        get_rule_expr((Node * )
        aggref->aggfilter, context, false);
    }
 ...
 */
static void
        get_agg_combine_expr(Node * node, deparse_context * context, void *
private)
{
Aggref *aggref;
Aggref *original_aggref =
...
get_windowfunc_expr(WindowFunc * wfunc, deparse_context * context) {
    ...
    if (wfunc->winstar)
        ...
        get_rule_expr((Node * )
    wfunc->args, context, true);

    if (wfunc->aggfilter != NULL) {
        appendStringInfoString(buf, ") FILTER (WHERE ");
        get_rule_expr((Node * )
        wfunc->aggfilter, context, false);
    }
 ...
 */
static void
        get_const_expr(Const * constval, deparse_context * context, int
showtype)
{
StringInfo buf = context->buf;
Oid typoutput;
...
appendStringInfo(buf,
"::%s",
format_type_with_typemod(constval
->consttype,
 ...
 */
static void
        get_tablefunc(TableFunc * tf, deparse_context * context, bool
showimplicit)
{
StringInfo buf = context->buf;
...
'(');
get_rule_expr((Node
*) tf->rowexpr, context, showimplicit);
appendStringInfoString(buf,
") PASSING (");
get_rule_expr((Node
*) tf->docexpr, context, showimplicit);
appendStringInfoChar(buf,
')');
...
" DEFAULT (");
get_rule_expr((Node
*) coldefexpr, context, showimplicit);
appendStringInfoChar(buf,
')');
}
if (colexpr != NULL)
                ...
                if (list_length(rte->functions) == 1 &&
                    ...
                    foreach(lc, rte->functions)
                    {
                        ...
                        if (!IsA(rtfunc->funcexpr, FuncExpr) ||
                            ((FuncExpr * )
                            rtfunc->funcexpr)->funcid != F_ARRAY_UNNEST ||
...
get_tablesample_def(TableSampleClause *tablesample, deparse_context *context) {
    ...
    if (tablesample->repeatable != NULL) {
        ...
        get_rule_expr((Node * )
        tablesample->repeatable, context, false);
        appendStringInfoChar(buf, ')');
    }
}
...
printSubscripts(ArrayRef *aref, deparse_context *context) {
    ...
    foreach(uplist_item, aref->refupperindexpr)
    {
        ...
        if (lowlist_item) {
            ...
            get_rule_expr((Node * )
            lfirst(lowlist_item), context, false);
            appendStringInfoChar(buf, ':');
            lowlist_item = lnext(lowlist_item);
        }
        /* If subexpression is NULL, get_rule_expr prints nothing */
        get_rule_expr((Node * )
        lfirst(uplist_item), context, false);
        appendStringInfoChar(buf, ']');
    }
}
