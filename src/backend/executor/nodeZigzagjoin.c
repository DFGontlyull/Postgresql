...
#include "executor/nodeZigzagjoin.h"
#include "miscadmin.h"
...
#include "utils/memutils.h"
  ...
  /*
   * States of the ExecZigzagJoin state machine
   */
...
#define EXEC_MJ_ENDINNER                11
	...
	*/
typedef struct ZigzagJoinClauseData {
...
#define MarkInnerTuple(innerTupleSlot, zigzagstate) \
    ExecCopySlot((zigzagstate)->mj_MarkedTupleSlot, (innerTupleSlot))
 ...
 */
static ZigzagJoinClause
MJExamineQuals(List *zigzagclauses,
	Oid *zigzagfamilies,
	Oid *zigzagcollations,
	int *zigzagstrategies,
	bool *zigzagnullsfirst,
	PlanState *parent) {
	ZigzagJoinClause clauses;
	int nClauses = list_length(zigzagclauses);
	int iClause;
	ListCell *cl;

	clauses = (ZigzagJoinClause)palloc0(nClauses * sizeof(ZigzagJoinClauseData));

	iClause = 0;
	foreach(cl, zigzagclauses)
	{
		OpExpr *qual = (OpExpr *)lfirst(cl);
		ZigzagJoinClause clause = &clauses[iClause];
		Oid opfamily = zigzagfamilies[iClause];
		Oid collation = zigzagcollations[iClause];
		StrategyNumber opstrategy = zigzagstrategies[iClause];
		bool nulls_first = zigzagnullsfirst[iClause];
		int op_strategy;
		Oid op_lefttype;
		Oid op_righttype;
		Oid sortfunc;
		...
		if (!IsA(qual, OpExpr))
			elog(ERROR, "zigzagjoin clause is not an OpExpr");
		...
		if (opstrategy == BTLessStrategyNumber)
			...
			elog(ERROR, "unsupported zigzagjoin strategy %d", opstrategy);
		...
		if (op_strategy != BTEqualStrategyNumber)    /* should not happen */
			elog(ERROR, "cannot zigzag using non-equality operator %u",
				qual->opno);
 ...
 */
static MJEvalResult
MJEvalOuterValues(ZigzagJoinState *zigzagstate) {
	ExprContext *econtext = zigzagstate->mj_OuterEContext;
	MJEvalResult result = MJEVAL_MATCHABLE;
	int i;
	MemoryContext oldContext;

	/* Check for end of outer subplan */
	if (TupIsNull(zigzagstate->mj_OuterTupleSlot))
	...
	econtext->ecxt_outertuple = zigzagstate->mj_OuterTupleSlot;

	for (i = 0; i < zigzagstate->mj_NumClauses; i++) {
		ZigzagJoinClause clause = &zigzagstate->mj_Clauses[i];
 ...
 */
static MJEvalResult
MJEvalInnerValues(ZigzagJoinState *zigzagstate, TupleTableSlot *innerslot) {
	ExprContext *econtext = zigzagstate->mj_InnerEContext;
	MJEvalResult result = MJEVAL_MATCHABLE;
	int i;
	MemoryContext oldContext;
 ...
 */
static int
MJCompare(ZigzagJoinState *zigzagstate) {
	int result = 0;
	bool nulleqnull = false;
	ExprContext *econtext = zigzagstate->js.ps.ps_ExprContext;
	int i;
	MemoryContext oldContext;
	 ...
	 */
	if (result == 0 &&
		(nulleqnull || zigzagstate->mj_ConstFalseJoin))
		result = 1;
...
MJFillOuter(ZigzagJoinState *node) {
	...
	if (ExecQual(otherqual, econtext)) {
		 ...
		 */
		MJ_printf("ExecZigzagJoin: returning outer fill tuple\n");
 ...
 */
static TupleTableSlot *
MJFillInner(ZigzagJoinState *node) {
	ExprContext *econtext = node->js.ps.ps_ExprContext;
	ExprState *otherqual = node->js.ps.qual;
	...
	if (ExecQual(otherqual, econtext)) {
		 ...
		 * return the slot containing it.
		 */
		MJ_printf("ExecZigzagJoin: returning inner fill tuple\n");
 ...
 * ----------------------------------------------------------------
 */
#ifdef EXEC_ZIGZAGJOINDEBUG

static void
ExecZigzagTupleDumpOuter(ZigzagJoinState *zigzagstate)
{
	TupleTableSlot *outerSlot = zigzagstate->mj_OuterTupleSlot;
...
static void
ExecZigzagTupleDumpInner(ZigzagJoinState *zigzagstate)
{
	TupleTableSlot *innerSlot = zigzagstate->mj_InnerTupleSlot;
...
static void
ExecZigzagTupleDumpMarked(ZigzagJoinState *zigzagstate)
{
	TupleTableSlot *markedSlot = zigzagstate->mj_MarkedTupleSlot;
...
static void
ExecZigzagTupleDump(ZigzagJoinState *zigzagstate)
{
	printf("******** ExecMergeTupleDump ********\n");

	ExecZigzagTupleDumpOuter(zigzagstate);
	ExecZigzagTupleDumpInner(zigzagstate);
	ExecZigzagTupleDumpMarked(zigzagstate);
 ...
 */
static TupleTableSlot *
ExecZigzagJoin(PlanState *pstate) {
	ZigzagJoinState *node = castNode(ZigzagJoinState, pstate);
	ExprState *joinqual;
	ExprState *otherqual;
	bool qualResult;
	int compareResult;
	...
	for (;;) {
		...
		case EXEC_MJ_INITIALIZE_OUTER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_INITIALIZE_OUTER\n");
			...
			case MJEVAL_ENDOFJOIN:
				...
				MJ_printf("ExecZigzagJoin: nothing in outer subplan\n");
				if (doFillInner) {
					/*
		...
		case EXEC_MJ_INITIALIZE_INNER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_INITIALIZE_INNER\n");
			...
			case MJEVAL_ENDOFJOIN:
				...
				MJ_printf("ExecZigzagJoin: nothing in inner subplan\n");
				if (doFillOuter) {
					/*
		...
		case EXEC_MJ_JOINTUPLES:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_JOINTUPLES\n");
			...
			if (qualResult) {
				...
				if (qualResult) {
					 ...
					 */
					MJ_printf("ExecZigzagJoin: returning tuple\n");
		...
		case EXEC_MJ_NEXTINNER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_NEXTINNER\n");
		...
		case EXEC_MJ_NEXTOUTER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_NEXTOUTER\n");
			...
			case MJEVAL_ENDOFJOIN:
				/* No more outer tuples */
				MJ_printf("ExecZigzagJoin: end of outer subplan\n");
				innerTupleSlot = node->mj_InnerTupleSlot;
		...
		case EXEC_MJ_TESTOUTER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_TESTOUTER\n");
		...
		case EXEC_MJ_SKIP_TEST:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_SKIP_TEST\n");
		...
		case EXEC_MJ_SKIPOUTER_ADVANCE:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_SKIPOUTER_ADVANCE\n");
		...
		case EXEC_MJ_SKIPINNER_ADVANCE:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_SKIPINNER_ADVANCE\n");
			...
			case MJEVAL_ENDOFJOIN:
				/* No more inner tuples */
				MJ_printf("ExecZigzagJoin: end of inner subplan\n");
				outerTupleSlot = node->mj_OuterTupleSlot;
		...
		case EXEC_MJ_ENDOUTER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_ENDOUTER\n");
		...
		case EXEC_MJ_ENDINNER:
			MJ_printf("ExecZigzagJoin: EXEC_MJ_ENDINNER\n");
			 ...
			 */
		default:
			elog(ERROR, "unrecognized zigzagjoin state: %d",
 ...
 */
ZigzagJoinState *
ExecInitZigzagJoin(ZigzagJoin *node, EState *estate, int eflags) {
	ZigzagJoinState *zigzagstate;

	/* check for unsupported flags */
	Assert(!(eflags & (EXEC_FLAG_BACKWARD | EXEC_FLAG_MARK)));

	MJ1_printf("ExecInitZigzagJoin: %s\n",
		"initializing node");

	/*
	 * create state structure
	 */
	zigzagstate = makeNode(ZigzagJoinState);
	zigzagstate->js.ps.plan = (Plan *)node;
	zigzagstate->js.ps.state = estate;
	zigzagstate->js.ps.ExecProcNode = ExecZigzagJoin;
	 ...
	 */
	ExecAssignExprContext(estate, &zigzagstate->js.ps);
	 ...
	 */
	zigzagstate->mj_OuterEContext = CreateExprContext(estate);
	zigzagstate->mj_InnerEContext = CreateExprContext(estate);

	/*
	 * initialize child expressions
	 */
	zigzagstate->js.ps.qual =
		ExecInitQual(node->join.plan.qual, (PlanState *)zigzagstate);
	zigzagstate->js.jointype = node->join.jointype;
	zigzagstate->js.joinqual =
		ExecInitQual(node->join.joinqual, (PlanState *)zigzagstate);
	zigzagstate->mj_ConstFalseJoin = false;
	/* zigzagclauses are handled below */
	...
	zigzagstate->mj_SkipMarkRestore = node->skip_mark_restore;

	outerPlanState(zigzagstate) = ExecInitNode(outerPlan(node), estate, eflags);
	innerPlanState(zigzagstate) = ExecInitNode(innerPlan(node), estate,
		zigzagstate->mj_SkipMarkRestore ?
		eflags :
	...
	if (IsA(innerPlan(node), Material) &&
		...
		!zigzagstate->mj_SkipMarkRestore)
		zigzagstate->mj_ExtraMarks = true;
	else
		zigzagstate->mj_ExtraMarks = false;

	/*
	 * tuple table initialization
	 */
	ExecInitResultTupleSlot(estate, &zigzagstate->js.ps);

	zigzagstate->mj_MarkedTupleSlot = ExecInitExtraTupleSlot(estate);
	ExecSetSlotDescriptor(zigzagstate->mj_MarkedTupleSlot,
		ExecGetResultType(innerPlanState(zigzagstate)));

	/*
	 * detect whether we need only consider the first matching inner tuple
	 */
	zigzagstate->js.single_match = (node->join.inner_unique ||
	...
	case JOIN_INNER:
	case JOIN_SEMI:
		zigzagstate->mj_FillOuter = false;
		zigzagstate->mj_FillInner = false;
		break;
	case JOIN_LEFT:
	case JOIN_ANTI:
		zigzagstate->mj_FillOuter = true;
		zigzagstate->mj_FillInner = false;
		zigzagstate->mj_NullInnerTupleSlot =
			ExecInitNullTupleSlot(estate,
				ExecGetResultType(innerPlanState(zigzagstate)));
		break;
	case JOIN_RIGHT:
		zigzagstate->mj_FillOuter = false;
		zigzagstate->mj_FillInner = true;
		zigzagstate->mj_NullOuterTupleSlot =
			ExecInitNullTupleSlot(estate,
				ExecGetResultType(outerPlanState(zigzagstate)));
		...
		if (!check_constant_qual(node->join.joinqual,
			&zigzagstate->mj_ConstFalseJoin))
			ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("RIGHT JOIN is only supported with zigzag-joinable join conditions")));
		break;
	case JOIN_FULL:
		zigzagstate->mj_FillOuter = true;
		zigzagstate->mj_FillInner = true;
		...
		if (!check_constant_qual(node->join.joinqual,
			...
			ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("FULL JOIN is only supported with zigzag-joinable join conditions")));
		break;
	default:
	 ...
	 */
	ExecAssignResultTypeFromTL(&zigzagstate->js.ps);
	ExecAssignProjectionInfo(&zigzagstate->js.ps, NULL);

	/*
	 * preprocess the merge clauses
	 */
	zigzagstate->mj_NumClauses = list_length(node->zigzagclauses);
	zigzagstate->mj_Clauses = MJExamineQuals(node->zigzagclauses,
		node->zigzagFamilies,
		node->zigzagCollations,
		node->zigzagStrategies,
		node->zigzagNullsFirst,
		(PlanState *)zigzagstate);

	/*
	 * initialize join state
	 */
	zigzagstate->mj_JoinState = EXEC_MJ_INITIALIZE_OUTER;
	zigzagstate->mj_MatchedOuter = false;
	zigzagstate->mj_MatchedInner = false;
	zigzagstate->mj_OuterTupleSlot = NULL;
	zigzagstate->mj_InnerTupleSlot = NULL;
 ...
 */
void
ExecEndZigzagJoin(ZigzagJoinState *node) {
	MJ1_printf("ExecEndZigzagJoin: %s\n",
...
void
ExecReScanZigzagJoin(ZigzagJoinState *node) {
	ExecClearTuple(node->mj_MarkedTupleSlot);
