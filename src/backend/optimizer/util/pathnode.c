...
/*
 * create_zigzagjoin_path
 * ZigzagJoin Plan 객체 생성용
 * 기존의 MergeJoin Cost 계산 방식을 따라가나,
 * PathType으로 T_ZigzagJoin을 부여함.
 */
MergePath *
create_zigzagjoin_path(PlannerInfo *root,
                      RelOptInfo *joinrel,
                      JoinType jointype,
                      JoinCostWorkspace *workspace,
                      JoinPathExtraData *extra,
                      ...
                      List *innersortkeys)
{
    ...
    pathnode->jpath.path.pathtype = T_ZigzagJoin;
    pathnode->jpath.path.parent = joinrel;
    pathnode->jpath.path.pathtarget = joinrel->reltarget;
    pathnode->jpath.path.param_info =
            get_joinrel_parampathinfo(root,
