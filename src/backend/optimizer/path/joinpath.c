...
static void
try_zigzagjoin_path(PlannerInfo *root,
                    RelOptInfo *joinrel,
                    Path *outer_path,
                    Path *inner_path,
                    List *pathkeys,
...
sort_inner_and_outer(PlannerInfo *root,
    ...
    foreach(l, all_pathkeys)
    {
        ...
        // 여기서 Parse한 구문이 Zigzag 일 경우, Zigzag Join으로 계획을 생성함.
        if (jointype == JOIN_ZIGZAG) {
            try_zigzagjoin_path(root,
                                joinrel,
                                outer_path,
                                inner_path,
                                merge_pathkeys,
