...
static void setNamespaceColumnVisibility(List *
...
static void setNamespaceLateralState(List *
...
rte = transformRangeSubselect(pstate, (RangeSubselect * )
n);
/* assume new rte is at end */
rtindex = list_length(pstate->p_rtable);
Assert(rte
...
rte = transformRangeFunction(pstate, (RangeFunction * )
n);
/* assume new rte is at end */
rtindex = list_length(pstate->p_rtable);
Assert(rte
...
rte = transformRangeTableFunc(pstate, (RangeTableFunc * )
n);
/* assume new rte is at end */
rtindex = list_length(pstate->p_rtable);
Assert(rte
...
IsA(n, RangeTableSample
))
{
/* TABLESAMPLE clause (wrapping some other valid FROM node) */
RangeTableSample *rts = (RangeTableSample * )
n;
Node *rel;
RangeTblRef *rtr;
RangeTblEntry *rte;
...
rte->
tablesample = transformRangeTableSample(pstate, rts);
return (Node *)
rtr;
}
else if (
 ...
 */
j->
larg = transformFromClauseItem(pstate, j->larg,
                               &l_rte,
                               &l_rtindex,
                               &l_namespace);
...
setNamespaceLateralState(l_namespace,
true, lateral_ok);

sv_namespace_length = list_length(pstate->p_namespace);
pstate->
p_namespace = list_concat(pstate->p_namespace, l_namespace);

/* And now we can process the RHS */
j->
rarg = transformFromClauseItem(pstate, j->rarg,
                               &r_rte,
                               &r_rtindex,
                               &r_namespace);
...
pstate->
p_namespace = list_truncate(pstate->p_namespace,
                            sv_namespace_length);
 ...
 */
expandRTE(l_rte, l_rtindex,
0, -1, false,
&l_colnames, &l_colvars);
expandRTE(r_rte, r_rtindex,
0, -1, false,
...
Assert(j
->usingClause == NIL);    /* shouldn't have USING() too */
...
Assert(j
->quals == NULL);    /* shouldn't have ON() too */
...
j->
quals = transformJoinUsingClause(pstate,
                                 l_rte,
                                 r_rte,
                                 l_usingvars,
                                 r_usingvars);
...
j->
quals = transformJoinOnClause(pstate, j, my_namespace);
}
else
{
/* CROSS JOIN: no quals */
...
k++)
pstate->
p_joinexprs = lappend(pstate->p_joinexprs, NULL);
pstate->
p_joinexprs = lappend(pstate->p_joinexprs, j);
Assert(list_length(pstate->p_joinexprs)
== j->rtindex);
...
setNamespaceColumnVisibility(my_namespace,
false);
 ...
 */
static void
setNamespaceColumnVisibility(List *
